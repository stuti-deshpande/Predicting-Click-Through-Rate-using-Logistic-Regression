...
modelGD = LogisticRegressionWithSGD.train(ClickTrain, iterations=15, step=1, miniBatchFraction=1, regType=None, validateData="False")

# Perform Testing on the Test Dataset and do prediction on the label
clickAndFeatures = ClickTest.map(lambda p: (float(modelGD.predict(p.features)), p.label))
