...
# Divide the trained model into Training and Validation data again
weights1 = [.6, .4]
seed1 = 15L

model_Train, model_Validate = ClickTrain.randomSplit(weights1, seed1)

#Train the model_Train data set for the L2 Regression
...
#modelL2_Train = LogisticRegressionWithSGD.train(model_Train, iterations=15, step=0.01, miniBatchFraction=0.01, regType='l2', regParam=0.01, validateData="True", convergenceTol=0.001)
#modelL2_Train = LogisticRegressionWithSGD.train(model_Train, iterations=50, step=0.01, miniBatchFraction=0.01, regType='l2', regParam=0.5, validateData="True", convergenceTol=0.001)

# Perform Testing on the Validate Dataset and get the best parameters
clickAndFeatures = model_Validate.map(lambda p: (float(modelL2_Train.predict(p.features)), p.label))
...
print ("Model: Validation-Dataset")
...
#-------------I ran the above code for three different models of "modelL2_Train" with following values for regParam and iterations"-----------
#---------------regParam= 0.01, 0.5, 50 
#---------------iterations= 15, 50, 100
#-------------Obtained the Highest Accuracy with regParam=50 and iterations=100 i.e. Accuracy= 0.8332
...
clickAndFeatures_test = ClickTest.map(lambda p: (float(modelL2_Train1.predict(p.features)), p.label))
...
print("Area under ROC = %s" % metrics_test.areaUnderROC)
...
print("False Positive Rate = %s" % fpr_test)